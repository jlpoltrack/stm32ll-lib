...
#if defined STM32C5
      LL_SPI_SetClockPolarity(SPIB_SPIx, LL_SPI_CLOCK_POLARITY_LOW);
      LL_SPI_SetClockPhase(SPIB_SPIx, LL_SPI_CLOCK_PHASE_1_EDGE);
#else
...
#if defined STM32C5
      LL_SPI_SetClockPolarity(SPIB_SPIx, LL_SPI_CLOCK_POLARITY_LOW);
      LL_SPI_SetClockPhase(SPIB_SPIx, LL_SPI_CLOCK_PHASE_2_EDGE);
#else
      LL_SPI_SetClockPolarity(SPIB_SPIx, LL_SPI_POLARITY_LOW);
...
#if defined STM32C5
      LL_SPI_SetClockPolarity(SPIB_SPIx, LL_SPI_CLOCK_POLARITY_HIGH);
      LL_SPI_SetClockPhase(SPIB_SPIx, LL_SPI_CLOCK_PHASE_1_EDGE);
#else
...
#else
    #warning SPIB: no clockspeed defined, 562 kHz selected!
    c5_speed = SPI_562p5KHZ;
#endif

    uint32_t c5_cfg2 = LL_SPI_MODE_MASTER | LL_SPI_FULL_DUPLEX | LL_SPI_MSB_FIRST |
                       LL_SPI_NSS_SOFT | c5_cpol | c5_cpha;
    uint32_t c5_cfg1 = LL_SPI_DATA_WIDTH_8_BIT | _spib_baudrate(c5_speed);
    LL_SPI_SetConfig(SPIB_SPIx, c5_cfg1, c5_cfg2);
    LL_SPI_DisableCRC(SPIB_SPIx);
  }
#endif // !SPIB_USE_SUBGHZSPI
